...
void addNewJoint(const std::string&                         name,
                 std::stack<std::shared_ptr<imog::Joint>>&  parents,
                 std::vector<std::shared_ptr<imog::Joint>>& joints,
                 bool                                       isEndSite = false) {

  auto j = std::make_shared<imog::Joint>(
...
namespace loader {
  ...
  std::shared_ptr<Motion> BVH(const std::string& bvhFilePath) {
    ...
    Timer timer("Parsing \"" + bvhFilePath + "\"");
